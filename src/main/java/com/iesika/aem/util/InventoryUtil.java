...
public class InventoryUtil {
	...
	public static int insertIntoMaidInventory(ItemStack stack, LMM_InventoryLittleMaid maidInventory, boolean sim) {
		final int limit = 64;
		if (stack == null || maidInventory == null) {
			return stack == null ? 0 : stack.stackSize;
		}
		for (int slot = 0; slot < maidInventory.maxInventorySize; slot++) {
	...
	//simがtrueならインベントリの中身を直接渡さずスタックのインスタンスをコピーして渡さないと正しく動かない
	public static int insertIntoInventory(ItemStack stack, IInventory inventory, int facing, boolean sim) {
		final int limit = 64;
		...
		if (inventory instanceof ISidedInventory) {
			...
			int[] accessible = sided.getAccessibleSlotsFromSide(facing);
